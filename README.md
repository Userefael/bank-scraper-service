...
| `OTP_FORCE_RESEND` | לא | האם ללחוץ על "שלחו קוד חדש" ברגע שדיאלוג הקוד נפתח. ברירת מחדל דולקת; `false` מכבה. |
...
הדיאלוג של הפועלים נפתח מעל דף ההתחברות ואינו משנה את הכתובת ואף לא את כותרת הדף, ולכן אי
אפשר לזהות אותו לפי URL. גם התיבות עצמן אינן מסגירות דבר: כפי שראינו בלוג מהבנק, חמש התיבות
הן `input` רגילים בלי `id`, בלי `name`, בלי `maxlength` ובלי `type="tel"`. מה שמזהה אותן הוא
הצירוף: שורה של ארבעה שדות אנונימיים גלויים ומעלה, שאינם שדות ההתחברות, בדף שהטקסט שלו קורא
כמו דרישת קוד ("קוד האימות", "כניסה חדשה ממחשב", "שלחו קוד חדש", "קוד קולי"). תיבות
`maxlength="1"` ושדה בודד ששמו נראה כמו קוד מזוהים גם הם, לגרסאות אחרות של הדיאלוג.

הבנק נצפה פותח את הדיאלוג בלי לשלוח הודעה כלל — התחברות ידנית מאותו חשבון בחלון פרטי
נכנסה בלי שום אתגר, בעוד השירות קיבל דיאלוג עם שני כפתורי שליחה גלויים. לכן ברירת המחדל היא
ללחוץ על "שלחו קוד חדש" ברגע שהדיאלוג נפתח: זה מה שמתחיל שליחה כשלא התחילה, וגם קושר את
הקוד שהמשתמש מקבל לסשן הזה ולא לניסיון קודם. `OTP_FORCE_RESEND=false` מכבה.

הסריקה עוברת על כל הפריימים, והאלמנטים שנמצאו מסומנים ב-`data-scraper-otp-field` כדי לפנות
אליהם בלי לנחש סלקטור. הקוד מוקלד תו-תו לתיבות, והכפתור נמצא לפי הטקסט שלו ולא לפי מחלקה,
בסדר עדיפות: "המשך" או "אישור" לפני "שלח", ו"כניסה" אחרון, כי כפתור ההתחברות של הדף שמאחורי
הדיאלוג נראה בדיוק אותו דבר. הלוגיקה שבוחרת את השדות היא פונקציה טהורה, `chooseOtpFields`,
ויש לה בדיקות.
