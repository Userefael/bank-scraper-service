...
| `HAPOALIM_OTP_INPUT_SELECTOR` | לא | סלקטור ידני לשדה הקוד בפועלים, אם הזיהוי האוטומטי מפספס. |
| `HAPOALIM_OTP_SUBMIT_SELECTOR` | לא | סלקטור ידני לכפתור השליחה בדף הקוד. |
...
השירות פותר את זה לפועלים בכך שהוא מנהל את ההתחברות בעצמו, ב-`src/scrapers/hapoalim-otp.js`.
...
שדה הקוד מזוהה לפי רשימת סלקטורים מסדר יורד של ספציפיות, כי אי אפשר לאמת את ה-HTML האמיתי
בלי חשבון אמיתי. אם הזיהוי מפספס, אפשר לכוון בלי שינוי קוד דרך `HAPOALIM_OTP_INPUT_SELECTOR`
ו-`HAPOALIM_OTP_SUBMIT_SELECTOR`. כשההתחברות נוחתת בדף שאינו מזוהה, השירות רושם ללוג שורת
`login_page_unrecognised` עם הכתובת, הכותרת, ושמות ותוויות השדות והכפתורים — בלי ערכים —
וזו השורה שממנה גוזרים את הסלקטור הנכון.
