...
**זו המגבלה החשובה ביותר בשירות הזה.** בגרסה 6.12.1 רק סקרייפר אחד בספרייה, `oneZero`, מממש
בפועל `triggerTwoFactorAuth` / `getLongTermTwoFactorToken`; בכל שאר הסקרייפרים המתודות האלה
זורקות מהמימוש הבסיסי. גם `otpCodeRetriever` קיים רק ב-`loginFields` של `oneZero`. כלומר
בשלושה עשר הספקים שהשירות חושף אין שום נקודת הזרקה לקוד OTP: אם הבנק דורש קוד באמצע ההתחברות,
הסקרייפר ממתין בדף עד ל-timeout ואין דרך תכנותית למסור לו את הקוד. `/connect` לא יחזיר
`requires_otp` עבורם, כי אין מה שיאותת על כך. השירות מזהה זאת בזמן ריצה בהשוואה ל-`BaseScraper.prototype`, כך שהזרימה תעבוד
...
* ספק בלי תמיכה דו-שלבית שדורש OTP → במקרה הטוב הספרייה מחזירה
  `TWO_FACTOR_RETRIEVER_MISSING` והשירות מחזיר `otp_required`; במקרה הפחות טוב ההתחברות
  נתקעת בדף הקוד עד לתקרת הזמן והשירות מחזיר `timeout`. בשני המקרים אי אפשר להשלים חיבור
  לספק כזה בלי סקרייפר ייעודי שנכתב עבורו.
